...
contract BondingCurve is Ownable, ICurve {
    ...
    function depositDai(uint256 daiAmount) external {
        //calculate one dai's price
        uint256 priceOfDAI = buyPrice(1*_BZZ_SCALE);
        uint256 _bzzAmount = daiAmount*1e18 / priceOfDAI;
        uint256 priceOfBzz = requiredCollateral(_bzzAmount);

        require(
            dai_.transferFrom(msg.sender, address(this), daiAmount),
            "Transferring collateral failed"
        );
        // Mints the user their tokens
        require(bzz_.mint(msg.sender, _bzzAmount), "Minting tokens failed");
        // Emitting event with all important info
        emit mintTokens(msg.sender, _bzzAmount, priceOfBzz, daiAmount);

    }

    function buyBzz(uint256 daiAmount) external view isActive returns(uint256) {
        //calculate one dai's price
        uint256 priceOfDAI = buyPrice(1*_BZZ_SCALE);
        uint256 _bzzAmount = daiAmount*1e18 / priceOfDAI;
        return _bzzAmount;
        ...
        returns (bool success)
    {
      ...
      function claimDAI(uint256 _amount, uint256 _minCollateralReward)
        external
        isActive
        mutex
        returns (bool success)
    ...
    function _spotPrice(uint256 _supply) internal pure returns (uint256) {
        return (_primitiveFunction(_supply + 1) - _primitiveFunction(_supply));
    ...
    function _mint(uint256 _amount, uint256 _currentSupply)
        ...
        returns (uint256)
    {
        uint256 deltaR = _primitiveFunction(_currentSupply + _amount) -
            _primitiveFunction(_currentSupply);
        return deltaR;
    }

    function bzzAmount(uint256 _amount, uint256 _currentSupply)
        internal
        pure
        returns (uint256)
    {
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 deltaR = _primitiveFunction(_currentSupply) -
            _primitiveFunction(_currentSupply - _amount);
        uint256 realized_price = deltaR / _amount;
    ...
    ///@dev test wrapper function.
    function helper(uint256 _x) public pure returns (uint256) {
        return _helper(_x);
    }

    function primitiveFunction(uint256 _s) public pure returns (uint256) {
        return _primitiveFunction(_s);
    }

    function spotPrice(uint256 _supply) public pure returns (uint256) {
    ...
    function burnBzz(uint256 _bzzAmount) external {
        require(_bzzAmount>0, "BondingCurve: invalid value");
        bzz_.burn(_bzzAmount);
    }
